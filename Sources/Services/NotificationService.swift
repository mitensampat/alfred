...
class NotificationService {
    ...
    //
    // Design tokens from Imperial design system:
    //   Canvas:    #f8f9fb   Surface:  #ffffff   Sunken: #f6f8fa
    //   Ink:       #24292f   Ink-2:    #57606a   Ink-3:  #8b949e
    //   Line:      #d0d7de   Coach:    #24292f   Growth: #1a7f37
    //   Warning:   #AF9358   Danger:   #cf222e   Info:   #AF9358
    //   Method:    #AF9358
    //   Display:   Newsreader, Georgia, serif
    //   Body:      Public Sans, -apple-system, sans-serif

    // Shared inline style constants
    private static let fontBody = "font-family: 'Public Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif;"
    private static let fontDisplay = "font-family: 'Newsreader', Georgia, 'Times New Roman', serif;"
    ...
    static func emailHead() -> String {
        ...
        <head>
            ...
            <link href="https://fonts.googleapis.com/css2?family=Public+Sans:wght@400;500;600;700&family=Newsreader:wght@400;500;600;700&display=swap" rel="stylesheet">
        </head>
        <body style="margin: 0; padding: 0; background-color: #f8f9fb; \(fontBody) color: #24292f;">
        ...
        <tr><td style="padding: 0 0 20px; text-align: left;">
            <span style="\(fontDisplay) font-size: 16px; font-weight: 400; color: #24292f; letter-spacing: 0.02em;">Coach Alfred</span>
        </td></tr>
        <!-- Main content card -->
        <tr><td style="background-color: #ffffff; border-radius: 0; border: 1px solid #d0d7de; padding: 32px 28px;">
        """
    }
    ...
    static func emailFoot() -> String {
        ...
        <tr><td style="padding: 20px 0 0; text-align: center;">
            <span style="\(fontBody) font-size: 11px; color: #8b949e;">Generated by <span style="\(fontDisplay) font-size: 12px; font-weight: 500; color: #57606a;">Coach Alfred</span> v\(AlfredApp.version) · \(Date().formatted(date: .abbreviated, time: .shortened))</span>
        </td></tr>
        </table>
        </td></tr>
        </table>
    ...
    private static func sectionHeader(_ text: String) -> String {
        ...
        <tr>
            <td style="\(fontBody) font-size: 11px; font-weight: 600; color: #57606a; text-transform: uppercase; letter-spacing: 0.06em; white-space: nowrap; padding-right: 12px;">\(text)</td>
            <td style="width: 100%;"><div style="height: 1px; background-color: #d0d7de;"></div></td>
        </tr>
        </table>
        """
    }
    ...
    private static func statRow(_ stats: [(value: String, label: String)]) -> String {
        ...
        for stat in stats {
            cells += """
            <td style="background-color: #f6f8fa; border-radius: 0; padding: 12px 8px; text-align: center; width: \(100 / max(stats.count, 1))%;">
                <div style="\(fontBody) font-size: 20px; font-weight: 600; color: #24292f;">\(stat.value)</div>
                <div style="\(fontBody) font-size: 11px; color: #57606a; margin-top: 2px;">\(stat.label)</div>
            </td>
            """
        }
        return """
    ...
    private static func coachingCard(emoji: String, label: String, insight: String, dotColor: String) -> String {
        ...
        <tr><td style="background-color: #f8f9fb; border-radius: 0; border: 1px solid #d0d7de; padding: 14px 16px;">
            <table role="presentation" cellpadding="0" cellspacing="0"><tr>
                ...
                <td style="padding-left: 8px;">
                    <div style="\(fontBody) font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; color: #57606a; margin-bottom: 4px;">\(emoji) \(label)</div>
                    <div style="\(fontBody) font-size: 14px; line-height: 1.65; color: #24292f;">\(insight)</div>
                </td>
            </tr></table>
        </td></tr>
        </table>
    ...
    private static func eventCard(time: String, title: String, location: String?, isExternal: Bool) -> String {
        ...
        if let loc = location, !loc.isEmpty {
            locationHtml = "<div style=\"\(fontBody) font-size: 12px; color: #57606a; margin-top: 3px;\">📍 \(loc)</div>"
        }
        let externalTag = isExternal ? " <span style=\"\(fontBody) display: inline-block; font-size: 10px; padding: 1px 6px; border-radius: 0; background-color: #fff8e1; color: #7c4a03;\">External</span>" : ""

        return """
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 6px;">
        <tr><td style="background-color: #f8f9fb; border-radius: 0; border-left: 3px solid #AF9358; padding: 10px 14px;">
            <div style="\(fontBody) font-size: 12px; color: #AF9358; font-weight: 500;">\(time)\(externalTag)</div>
            <div style="\(fontBody) font-size: 14px; font-weight: 600; color: #24292f; margin-top: 2px;">\(title)</div>
            \(locationHtml)
        </td></tr>
        </table>
        """
    ...
    private static func itemCard(title: String, description: String, meta: String, accentColor: String, emoji: String) -> String {
        return """
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 8px;">
        <tr><td style="background-color: #f8f9fb; border-radius: 0; border-left: 3px solid \(accentColor); padding: 12px 16px;">
            <div style="\(fontBody) font-size: 14px; font-weight: 600; color: #24292f;">\(emoji) \(title)</div>
            <div style="\(fontBody) font-size: 13px; line-height: 1.55; color: #24292f; margin-top: 4px;">\(description)</div>
            <div style="\(fontBody) font-size: 12px; color: #8b949e; margin-top: 6px;">\(meta)</div>
        </td></tr>
        </table>
        """
    }
    ...
    private static func simpleCard(content: String) -> String {
        return """
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 6px;">
        <tr><td style="background-color: #f6f8fa; border-radius: 0; padding: 10px 14px;">
            <div style="\(fontBody) font-size: 13px; line-height: 1.55; color: #24292f;">\(content)</div>
        </td></tr>
        </table>
        """
    }
    ...
    private static func recCard(_ text: String) -> String {
        return """
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 6px;">
        <tr><td style="background-color: #fff8e1; border-radius: 0; padding: 10px 14px;">
            <div style="\(fontBody) font-size: 13px; line-height: 1.5; color: #AF9358;">\(text)</div>
        </td></tr>
        </table>
        """
    }
    ...
    private func formatBriefing(_ briefing: DailyBriefing) -> (markdown: String, html: String) {
        ...
        // Title
        html += "<div style=\"\(Self.fontDisplay) font-size: 22px; font-weight: 400; color: #24292f; margin-bottom: 4px;\">Daily Briefing</div>"
        html += "<div style=\"\(Self.fontBody) font-size: 12px; color: #8b949e; margin-bottom: 8px;\">\(briefing.date.formatted(date: .long, time: .omitted))</div>"
        ...
        if !briefing.actionItems.isEmpty {
            ...
            for item in briefing.actionItems.prefix(10) {
                ...
                case .critical:
                    accentColor = "#cf222e"; priorityEmoji = "🔴"
                case .high:
                    accentColor = "#cf222e"; priorityEmoji = "🟠"
                case .medium:
                    accentColor = "#AF9358"; priorityEmoji = "🟡"
                default:
                    accentColor = "#1a7f37"; priorityEmoji = "🟢"
                }
    ...
    private func formatAttentionReport(_ report: AttentionDefenseReport) -> (markdown: String, html: String) {
        ...
        var html = Self.emailHead()
        html += "<div style=\"\(Self.fontDisplay) font-size: 22px; font-weight: 400; color: #24292f; margin-bottom: 4px;\">Attention Defense</div>"
        html += "<div style=\"\(Self.fontBody) font-size: 12px; color: #8b949e; margin-bottom: 8px;\">Generated at \(report.currentTime.formatted(date: .omitted, time: .shortened))</div>"
        ...
        for item in report.mustDoToday {
            ...
            let accentColor = item.priority == .high ? "#cf222e" : "#AF9358"
            html += Self.itemCard(
                title: item.title,
                description: item.description,
    ...
    private func formatAgentDigest(_ digest: AgentDigest) -> (markdown: String, html: String) {
        ...
        var html = Self.emailHead()
        html += "<div style=\"\(Self.fontDisplay) font-size: 22px; font-weight: 400; color: #24292f; margin-bottom: 4px;\">Agent Digest</div>"
        html += "<div style=\"\(Self.fontBody) font-size: 12px; color: #8b949e; margin-bottom: 8px;\">\(dateFormatter.string(from: digest.date))</div>"
        ...
        for activity in digest.agentActivity {
            ...
            if let insight = activity.keyInsight {
                insightHtml = "<div style=\"\(Self.fontBody) font-size: 13px; color: #24292f; margin-top: 8px; padding: 8px 10px; background-color: #ffffff; border-radius: 0;\">💡 \(insight)</div>"
            }

            html += """
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 8px;">
            <tr><td style="background-color: #f6f8fa; border-radius: 0; padding: 14px 16px;">
                <div style="\(Self.fontBody) font-size: 14px; font-weight: 600; color: #24292f; margin-bottom: 6px;">\(activity.agentType.displayName) Agent</div>
                <div style="\(Self.fontBody) font-size: 12px; color: #57606a;">\(activity.decisionsCount) decisions · \(successPct)% success</div>
                \(insightHtml)
            </td></tr>
            </table>
            """
        ...
        if !digest.upcomingFollowups.isEmpty {
            ...
            for followup in digest.upcomingFollowups.prefix(5) {
                ...
                let accentColor = followup.isOverdue ? "#cf222e" : "#d0d7de"
                html += Self.itemCard(
